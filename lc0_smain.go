...
func (t *GameTask) Run() {
	...
	var err error
	err = nextGame(t.cli, t.ctr, t.gpuIteration)
	if err != nil {
		if err.Error() == "retry" {
...
func (c *cmdWrapper) launch(networkPath string, otherNetPath string, args []string, input bool, gpuIteration ... int) {
	...
	if *gpu >= 0 {
		...
		sGpu = fmt.Sprintf(",gpu=%v", gpuIteration[0])
	}
...
func playMatch(httpClient *http.Client, ngr client.NextGameResponse, baselinePath string, candidatePath string, params []string, gpuIteration int) (*client.NextGameResponse, error) {
	...
	c := createCmdWrapper()
	c.launch(candidatePath, baselinePath, params /* input= */, false, gpuIteration)
...
func nextGame(httpClient *http.Client, count int, gpuIteration int) error {
	var nextGame client.NextGameResponse
	var err error
	fmt.Println("using gpu no %v", gpuIteration)
	...
	if nextGame.Type == "match" {
		...
		possibleNextGame, err := playMatch(httpClient, nextGame, networkPath, candidatePath, serverParams, gpuIteration)
		if err != nil {
...
func main() {
	...
	for i := 0; ; i++ {
		task := &GameTask{httpClient, i, gpuIteration}
		pool.Execute(task)
		if i+1 % gpunum == 0 {
